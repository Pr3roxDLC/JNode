...
public class Script {
    ...
    public void drawScriptBlocks(Graphics2D g) {
        ...
        subScripts.forEach(n -> {
            ...
            n.getBlocks().forEach(block -> drawBlock2(block, g, n.getX(), n.getY(), extraWidth.get()));

            g.setColor(Color.BLACK);
            GuiUtil.getBoundingBoxes(n).forEach(box -> g.drawRect(box.getValue().x, box.getValue().y, box.getValue().width, box.getValue().height));
        });
    ...
    private void drawBlock2(Block block, Graphics2D g, int x, int y, int extraWidth) {
        ...
        if (block instanceof ControlBlock) {
            ...
            xOffset += 10;
            ((ControlBlock) block).getChildren().forEach(child -> drawBlock2(child, g, x, y, extraWidth));
            xOffset -= 10;
